...
description: 用 Remotion(代码化)把一个素材目录 + 创意想法做成产品宣传片/介绍视频/展厅循环片。当用户提供素材目录(图片/实物图/产品视频/小程序截图等)并想生成产品宣传视频、宣传片、产品介绍视频、展厅播放视频时使用。涵盖分镜设计、素材处理、阿里云百炼 CLI 脚本/配音/可选快乐马 AI 视频补镜、火山引擎 TTS 备选、背景音乐合成、渲染与自检的完整流程。
---

# 产品宣传片制作 (Remotion)

把「素材目录 + 创意想法」做成一支可直接播放的 MP4 宣传片。主体用 Remotion（React 代码化视频）本地渲染，可复现、易微调；脚本、TTS、少量补镜优先用阿里云百炼 CLI `bl`。

适合：图片/实物图/小程序截图/短视频为主、文字+动效+配音的 motion-graphics 型宣传片（展厅循环、产品介绍、发布会片头）。
不适合：需要大量真人实拍/复杂运镜的镜头。少量缺失镜头可用百炼快乐马模型补齐，再回到 Remotion 合成。

## 环境前置
- `node`(>=18) `npm` `ffmpeg/ffprobe` `python3`(含 numpy、requests)。开工前 `which node ffmpeg python3` 确认。
- `bl` CLI 已安装并登录：`bl --version`、`bl auth status`。使用 `bl` 前按 `bailian-cli` skill 的版本预检规则确认版本。
...
- **风格直接给现成 3 套预设选**(见 `styles/STYLES.md`)：`warm` 温情家庭向 / `guochao` 活力国潮 / `tech` 商务科技。每套都已实现好配色+装饰+分镜，选定后在套用风格步骤直接使用，不用从零设计视觉。需要全新风格再以最接近的一套为基底改。
...
### 5. 可选：百炼快乐马补镜
- 素材缺关键镜头时可用 `bl video generate` / `bl video ref` 生成补镜；素材足够时可跳过，直接用 Remotion 动效。
- **不要在 skill 中限制快乐马视频生产长度**。`--duration` 按用户需求、预算和百炼模型支持范围设置；需要更长成片时生成多个片段后交给 Remotion/ffmpeg 合成。
- 为节省试错成本，首次测试建议用 720P、5 秒小样确认方向，确认后再按正式需求调整参数。
- 文生视频示例（低成本测试小样，可调整时长）：
```bash
bl video generate \
...
```
- 图生视频示例（低成本测试小样；本地图片可直接传给 `--image`，CLI 会自动上传临时文件）：
```bash
bl video generate \
...
```
- 生成后用 `ffprobe` 看时长/编码；若是 HEVC 或浏览器不稳，按第 4 步转 H.264。
...
- **不要把 5 秒小样写成生产限制**。快乐马补镜的 `--duration` 由用户需求、预算和模型支持范围决定；长片可由多个 AI 片段、真实素材和 Remotion/ffmpeg 拼接完成。
- **AI 视频提示词避免文字/Logo**，否则容易生成错字或变形标识。品牌名、卖点、字幕统一用 Remotion 叠加。
- **火山 3001** 多为账号授权问题，不是代码问题，按配音步骤排查。
...
## 文件地图
- `scripts/gen_tts_bailian.py` 阿里百炼 CosyVoice TTS(首选, 调 `bl speech synthesize`)
...
- `config/bailian.env` 百炼 TTS 参数  ·  `config/volcano.env` 火山标准TTS凭证  ·  `config/seed.env` 大模型TTS凭证
